...
class AssemblyDialog(QDialog):
    ...
    def open_assembly_tool(self):
        """Open the Assembly Laser Annotation Tool without closing the whole app"""
        self.disconnect_tcp()
...
class ScrewDialog(QDialog):
    def __init__(self, parent=None, block_id=None, block_name=None, initial_config=None):
        ...
        self.setModal(False) # Use ApplicationModal instead of WindowModal
        ...
        self.length_spinbox.setRange(1, 200)
    ...
    def open_assembly_tool(self):
        """Open Screw Location Tool safely (single-instance, non-blocking)"""
        try:
            from ui.components.assembly_laser import MainWindow as AssemblyLaserMainWindow

            # 如果已经开着，就直接切到前面
            if self.assembly_tool_window is not None:
                try:
                    if self.assembly_tool_window.isVisible():
                        self.assembly_tool_window.raise_()
                        self.assembly_tool_window.activateWindow()
                        return
                except RuntimeError:
            ...
            self.assembly_tool_window.setWindowModality(Qt.NonModal)
            self.assembly_tool_window.setAttribute(Qt.WA_DeleteOnClose, True)

            self.assembly_tool_window.destroyed.connect(self._on_screw_tool_closed)
    ...
    def open_assembly_tool_2(self):
        """Open Screw Location Tool 2 safely (single-instance, non-blocking)"""
        try:
            from ui.components.assembly_laser import MainWindow as AssemblyLaserMainWindow
    ...
    def _on_screw_tool_closed(self, *args):
        """Restore ScrewDialog after screw tool closed"""
        self.assembly_tool_window = None
        self.setEnabled(True)
        self.show()
        self.raise_()
